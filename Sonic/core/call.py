...
import config
from Sonic import LOGGER, YouTube, app
...
from Sonic.utils.formatters import check_duration, seconds_to_min, speed_converter
from Sonic.utils.inline.play import qe_markup, stream_markup
...
class Call(PyTgCalls):
    ...
    async def change_stream(self, client: PyTgCalls, chat_id: int):
        ...
        try:
            ...
            try:
                await popped["mystic"].edit_reply_markup(
                    reply_markup=InlineKeyboardMarkup(qe_markup(_))
                )
            except:
                pass
            if not check:
        ...
        if "live_" in queued:
            ...
            if videoid == "telegram":
                ...
                run = await app.send_photo(
                    ...
                    caption=_["stream_1"].format(
                        config.SUPPORT_GROUP, title[:20], check[0]["dur"], user
                    ),
                    reply_markup=InlineKeyboardMarkup(button),
                )
                db[chat_id][0]["mystic"] = run
