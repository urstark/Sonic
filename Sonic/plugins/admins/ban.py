...
#
#  Made with dedication and love by urstark
# -----------------------------------------------
...
async def ban_command_handler(client, message):
    ...
    # Improved admin check using SUDOERS and cached adminlist
    ...
    # If not sudo, check for restrict privilege (optional but recommended)
    if admin_id not in SUDOERS:
        member = await chat.get_member(admin_id)
        if not (member.status in [enums.ChatMemberStatus.ADMINISTRATOR, enums.ChatMemberStatus.OWNER] and member.privileges.can_restrict_members):
            return await message.reply_text("You don't have permission to ban someone")
