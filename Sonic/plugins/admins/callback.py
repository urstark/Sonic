...
from Sonic.utils.formatters import seconds_to_min
from Sonic.utils.inline import close_markup, stream_markup, stream_markup_timer
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Sonic.pause_stream(chat_id)
        ...
        await Sonic.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
        ...
        if "live_" in queued:
            ...
            if videoid == "telegram":
                ...
                run = await CallbackQuery.message.reply_photo(
                    ...
                    caption=_["stream_1"].format(
                        config.SUPPORT_GROUP, title[:20], duration, user
                    ),
                    reply_markup=InlineKeyboardMarkup(button),
                )
                db[chat_id][0]["mystic"] = run
